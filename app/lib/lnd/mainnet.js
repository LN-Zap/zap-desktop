...
/**
 * Wrapper class for Lnd to run and monitor it in Mainnet mode.
 * @extends EventEmitter
 */
class Mainnet extends EventEmitter {
  lndConfig: LndConfig
  process: any
  ...
  /**
   * Start the Lnd process in Mainnet mode.
   * @return {Number} PID of the Lnd process that was started.
   */
  async start() {
    if (this.process) {
      return Promise.reject(
        new Error('Mainnet process with PID ${this.process.pid} already exists.')
      )
    }

    mainLog.info('Starting lnd in mainnet mode')
    ...
    mainLog.info(
      'Spawning Mainnet process: %s %s',
      this.lndConfig.binaryPath,
      mainnetArgs.filter(v => v != '').join(' ')
    )

    // Spawn lnd process.
    this.process = spawn(this.lndConfig.binaryPath, mainnetArgs)
      .on('error', error => {
    ...
    // Listen for when mainnet prints data to stderr.
    ...
    // Listen for when mainnet prints data to stdout.
    this.process.stdout.pipe(split2()).on('data', line => {
      ...
      if (this.is(CHAIN_SYNC_PENDING) || this.is(CHAIN_SYNC_WAITING)) {
        const match = line.match(/Waiting for chain backend to finish sync, start_height=(\d+)/)
  ...
  kill(signalName: string = 'SIGINT') {
    if (this.process) {
      mainLog.info('Killing Mainnet process...')
  ...
  setCurrentBlockHeight(height: number | string) {
    ...
    const changed = Mainnet.incrementIfHigher(this, 'currentBlockHeight', heightAsNumber)
    if (changed) {
  ...
  setLndBlockHeight(height: number | string) {
    const heightAsNumber = Number(height)
    const changed = Mainnet.incrementIfHigher(this, 'lndBlockHeight', heightAsNumber)
    if (changed) {
  ...
  setLndCfilterHeight(height: number | string) {
    const heightAsNumber = Number(height)
    const changed = Mainnet.incrementIfHigher(this, 'lndCfilterHeight', heightAsNumber)
    if (changed) {
